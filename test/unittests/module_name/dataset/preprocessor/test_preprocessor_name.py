import os
import sys
import unittest


sys.dont_write_bytecode = True

MODULE_DIR = os.environ.get('MODULE_DIR', None)
if MODULE_DIR is not None:
    sys.path = [MODULE_DIR] + sys.path
else:
    raise ValueError("MODULE_DIR environment variable not set!")



class TestPreprocessorName(unittest.TestCase):
    def test_build_by_config(self):
        from module_name.dataset.preprocessor import PREPROCESSOR_BUILDER
        config = {
            "name": "PreprocessorName",
            "args": {},
        }
        name = config["name"]
        args = config["args"]
        
        preprocessor = PREPROCESSOR_BUILDER.build(name, **args)
        self.assertIsNone(preprocessor(None))

    def test_build(self):
        from module_name.dataset.preprocessor.preprocessor_name import PreprocessorName
        preprocessor = PreprocessorName()
        self.assertIsNone(preprocessor(None))


if __name__ == "__main__":
    unittest.main()